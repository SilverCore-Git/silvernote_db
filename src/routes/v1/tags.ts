...
router.get("/length", async (req, res) => {
  try {
    const db = getDB();
    const length = await db.collection("tags").countDocuments();
    res.json({ length });
  } catch (err) {
    res.status(500).json({ error: "Error on get tag length" });
  }
});
...
router.get("/get/byuserid/:userid/WithIndex/:start/:end", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "Error fetching indexed tags" });
  }
...
router.post("/update", async (req, res) => {
  try {
    ...
    res.status(201).json({ uuid: tag.uuid });
  } catch (err:any) {
